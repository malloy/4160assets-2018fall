...
public:
  ...
  const Vector2f getCentroid() const {
    return getPosition() + Vector2f(getFrame()->getWidth(), getFrame()->getHeight()) / 2;
  }

  void accelerate(Uint32 ticks, const Vector2f& a) { 
    setVelocity(getVelocity() + ticks * 0.001f * a);
  }
