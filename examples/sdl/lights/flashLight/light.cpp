    ...
    for (currX = 0; currX < w_wid; currX += dif_grid_wid) {
        for (currY = 0; currY < w_hgt; currY += dif_grid_hgt) {
            ...
            for(plitr = ll.begin(); plitr != ll.end(); plitr++) {
                ...
                Triple distvec = foreLight_pos - cent;
                dist = distvec.len();

                fals_brig = currLight.bright() / (dist*dist);

                if (count >  0 ||  (cX >= foreLight_pos.getX() 
                                &&  cY >= foreLight_pos.getY() + distvec.getX()*1 
                                && cY <= foreLight_pos.getY() - distvec.getX()*1 )) // && cY <= foreLight_pos.getY() + yDif &&  cY >= foreLight_pos.getY() - yDif))
                {
